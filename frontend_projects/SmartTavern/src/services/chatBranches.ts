...
async function postJSON<T = any>(path: string, body: Record<string, any> = {}): Promise<T> {
  ...
  try {
    ...
    const err = new Error(
      `[ChatBranches] Network error: ${(networkError as Error)?.message || networkError}`,
    ) as ErrorWithDetails
  ...
  try {
    ...
    const err = new Error(
      `[ChatBranches] Invalid JSON response (${resp.status}): ${text?.slice(0, 200)}`,
    ) as ErrorWithDetails
  ...
  if (!resp.ok) {
    const err = new Error(
      `[ChatBranches] HTTP ${resp.status}: ${(data && (data.message || data.error)) || 'Unknown error'}`,
    ) as ErrorWithDetails
    err.status = resp.status
    err.url = url
    err.details = data
...
const ChatBranches = {
  ...
  async getLatestMessageByFile(
    ...
    const res = await postJSON<LatestMessageResponse>(
  ...
  async appendMessage({
    ...
    role,
    content,
  }: AppendMessageParams): Promise<AppendMessageResponse> {
    if (!file) {
  ...
  async updateMessage({
    ...
    node_id,
    content,
  }: UpdateMessageParams): Promise<UpdateMessageResponse> {
    if (!file) {
  ...
  async retryBranch({
    ...
    role,
    content,
  }: RetryBranchParams): Promise<RetryBranchResponse> {
    if (!file) {
      throw new Error('[ChatBranches] retryBranch: file is required')
    }
    if (!new_node_id || !retry_node_id || !role || content === undefined) {
      throw new Error(
        '[ChatBranches] retryBranch: new_node_id, retry_node_id, role, content are required',
      )
    }
  ...
  async retryUserMessage({
    file,
    user_node_id,
  }: RetryUserMessageParams): Promise<RetryUserMessageResponse> {
    if (!file) {
