...
async function bootstrapApp(): Promise<void> {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (isProd && fromLS && /^https?:\/\/localhost[:/]/.test(fromLS)) {
          try {
            localStorage.removeItem('st.backend_base')
          } catch (_) {}
        } else {
  ...
  ThemeManager.init().finally(async () => {
    ...
    try {
      registerHomeMenuBuiltins()
    } catch (e) {
      console.warn('[bootstrap] homeMenu', e)
    }
    try {
      registerSidebarBuiltins()
    } catch (e) {
      console.warn('[bootstrap] sidebar', e)
    }
    ...
    try {
      ...
      if (typeof window !== 'undefined')
        (window as any).disposeCompletionBridge = disposeCompletionBridge
    } catch (e) {
      ...
      async function loadPluginsFromSwitch(): Promise<void> {
        ...
        try {
          ...
          if (Array.isArray(sw?.enabled)) {
            ...
            Host.pushToast?.({
              type: 'error',
              message: i18n.t('bootstrap.missingPluginSwitch'),
              timeout: 2800,
            })
            return
          }
          ...
          Host.pushToast?.({
            type: 'error',
            message: i18n.t('bootstrap.pluginSwitchReadFailed'),
            timeout: 2600,
          })
          return
        }
        ...
        try {
          ...
          for (const er of errs) {
            try {
              Host.pushToast?.({
                type: 'warning',
                message: i18n.t('bootstrap.pluginDirMissing', { file: er?.file || '' }),
                timeout: 2600,
              })
            } catch (_) {
              // Ignore errors
          ...
          for (const dir of toLoadDirs) {
            try {
              ...
              Host.pushToast?.({
                type: 'error',
                message: i18n.t('bootstrap.pluginLoadFailed', { dir }),
                timeout: 2200,
              })
            }
          }
          if (loaded > 0) {
            Host.pushToast?.({
              type: 'success',
              message: i18n.t('bootstrap.pluginsLoaded', { count: loaded }),
              timeout: 2000,
            })
          }
        } catch (e) {
