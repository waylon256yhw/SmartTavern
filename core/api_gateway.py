...
class _IgnoreWinProactorConnLostFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if "_ProactorBasePipeTransport._call_connection_lost" in msg:
            return False
        return "WinError 10054" not in msg
...
class APIRouter:
    ...
    def add_endpoint(self, path: str, method: str, handler: Callable, **kwargs):
        """添加API端点"""
        endpoint = APIEndpoint(path=path, method=method.upper(), handler=handler, **kwargs)
        self.endpoints.append(endpoint)
        logger.info(f"✓ 注册API端点: {method.upper()} {path}")

    def add_middleware(self, name: str, handler: Callable, priority: int = 0):
        """添加中间件"""
        middleware = MiddlewareConfig(name=name, handler=handler, priority=priority)
...
class Middleware:
    ...
    async def auth_middleware(request: Request, call_next):
        ...
        if not auth:
            return JSONResponse(
                status_code=401, content={"error_code": "UNAUTHORIZED", "message": "缺少Authorization头"}
            )
        return await call_next(request)
...
class APIGateway:
    ...
    def _load_config(
        ...
        if config_file:
            ...
            # 尝试多个可能的配置文件位置
            possible_paths = ["api-config.json", "config/api-config.json", "backend_projects/api-config.json"]
    ...
    def _extract_gateway_config_from_project(self, project_config: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "api": {"prefix": prefix, "auto_discovery": True, "documentation": {"enabled": True, "url": "/docs"}},
            "websocket": {
                "enabled": websocket_config.get("enabled", True),
                "path": websocket_config.get("path", "/ws"),
            },
            "static_files": {"enabled": False, "directory": "", "url_prefix": "/static"},
        }
    ...
    def _setup_default_routes(self):
        """设置默认路由"""
        # 健康检查端点
        self.router.add_endpoint("/health", "GET", self._health_check_handler, tags=["system"], summary="健康检查")

        # API信息端点
        self.router.add_endpoint("/info", "GET", self._api_info_handler, tags=["system"], summary="API信息")
                    ...
                    try:
                        ...
                        try:
                            ...
                            if websocket.client_state != WebSocketState.DISCONNECTED:
                                error_response = {"type": "error", "error": "消息处理失败", "detail": str(e)}
        ...
        try:
            ...
            if msg_type == "ping":
                ...
                if func:
                    ...
                    return {"type": "function_result", "function": func_path, "success": True, "result": result}
                else:
                    return {
                        "type": "function_result",
                        "function": func_path,
...
def get_api_gateway(
    config: GatewayConfig | None = None, config_file: str | None = None, project_config: dict[str, Any] | None = None
) -> APIGateway:
    """获取API网关单例"""
